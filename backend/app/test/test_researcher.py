...
def _smoke_check(result: Dict[str, Any]) -> None:
    ...
    for f in result["findings"]:
        assert isinstance(f, Finding), f"finding phải là Pydantic Finding: {f!r}"
        assert f.source_url.startswith("http"), f"finding thiếu source_url: {f!r}"
...
def _print_summary(payload: Dict[str, Any]) -> None:
    ...
    for f in payload["findings"]:
        print(f"  - {f['source_title'][:50]} | date={f['published_at']}")
...
async def main() -> dict:
    ...
    # Giả lập state như Orchestrator trả về (plan là Pydantic object như graph thật)
    state = {
        "topic": TOPIC,
        "plan": ResearchPlan(topic=TOPIC, sub_queries=SUB_QUERIES or [TOPIC]),
        "retry_count": 0,
    }
